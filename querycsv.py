...
def preprocess(original_table_list, conds):
	...
	for i in range(len(conds)):
		...
		for j in range(len(ret_list)):
			if conds[i][0] in ret_list[j] and conds[i][3] == "AND":
				ret_list[j] = ret_list[j][operators(ret_list[j], conds[i][0], conds[i][2], conds[i][1])]
				flag = True
			elif conds[i][0] in ret_list[j] and conds[i][3] == "OR":
				flag = True
				temp = pd.DataFrame()
				for table in original_table_list:
					if conds[i][0] in table:
						temp = table[operators(table, conds[i][0], conds[i][2], conds[i][1])]
						break
				ret_list[j] = pd.concat([ret_list[j], temp], axis = 0)
